...
test('creating a resource for a directory', async () => {
  fs.mkdirSync(`${tmp.tmpdir}/data/electionFixture/cvr-files`, {
    recursive: true,
  });
  const tmpDirName = tmp.dirSync({
    template: `${tmp.tmpdir}/data/electionFixture/cvr-files/resource-XXXXXX`,
  }).name;
  const result = await convert({
    path: tmpDirName,
    tsPath: `${tmp.tmpdir}/src/data/electionFixture/cvr-files/resource.ts`,
